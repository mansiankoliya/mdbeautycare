  ...
  return (
    <div className={`${data.filterListDropdown ? "" : "hidden"} my-4`}>
      ...
      <div className="w-full flex flex-col">
        <div className="font-medium py-2 colorclass">Filter by price</div>
        <div className="flex justify-between items-center">
          <div className="flex flex-col space-y-2  w-2/3 lg:w-2/4">
            <label htmlFor="points" className="text-sm">
              Price (between 0 and 10000₹):{" "}
              <span className="font-semibold colorclass">{range}.00₹</span>{" "}
            </label>
            <input
              value={range}
              className="slider"
          ...
          <div onClick={(e) => closeFilterBar()} className="cursor-pointer">
            <svg
              className="w-8 h-8 colorclass hover:bg-gray-200 rounded-full p-1"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
